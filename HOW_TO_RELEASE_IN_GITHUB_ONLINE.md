...
Make sure all changes made inside the AI Studio sandbox (including our update to version `0.1.2` in `package.json`) are exported to your GitHub repository.
...
5. Click the button to export/push files to your **`main`** branch. This updates the remote repository to version `0.1.2`.
...
4. Click the box labeled **Choose a tag**:
   - Type in **`v0.1.2`** *(make sure it starts with a lowercase "v" and matches the exact version in `package.json`)*.
   - Click the blue option that appears below it: **Create new tag: v0.1.2 on publish**.
5. Keep the **Target** dropdown set to **`main`**.
6. Enter a Title (e.g., `v0.1.2 Release`) and write any high-level notes in the description area.
...
Now you will publish the release, which instantly instructs GitHub to create the `v0.1.2` tag on your repository. This tag push is what triggers GitHub Actions to compile the applications.
...
2. You will see a newly running workflow called **Build/Release Electron App** triggered by your new tag `v0.1.2`.
...
4. Once completed, <a href="https://github.com/JON99999/Interstitial-er/releases" target="_blank" rel="noopener noreferrer">return to your live Releases page on GitHub</a>. The packaged installer files (`.dmg`, `.exe`, `.zip`) will now be neatly listed as download attachments under the `v0.1.2` assets foldout list!
